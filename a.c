...
void guardarcurp(struct curp_registro registros[], int *num_registros, char curp[])
{
    ...
    strcpy(registros[*num_registros].CURP, curp);
}
char siglo(int year)
{
    if (year < 2000)
...
void fechadenacimiento(struct curp_registro registros[], int *num_registros, char fecha[], char diferenciador[])
{
    ...
    strcpy(registros[*num_registros].Fecha, fecha);
    *diferenciador = siglo(year);
    edad = calcularEdad(year, mes, dia);
    registros[*num_registros].Edad = edad;
}
void genero(struct curp_registro registros[], int *num_registros, char sexo[])
{
    int num;
    ...
    strcpy(registros[*num_registros].sexo, gener_sexo[num]);
}
void estados(struct curp_registro registros[], int *num_registros, char abreviacion[])
{
    char estado[20];
    ...
    strcpy(registros[*num_registros].Lugar, estado);
    for (i = 0; i < 32; i++)
    {
...
void nombres(struct curp_registro registros[], int *num_registros, char cuadro1[], char finalnombre[])
{
    ...
    strcpy(registros[*num_registros].Nombre, nom);
    ...
    if (segundo == 1)
    {
        ...
        strcpy(registros[*num_registros].Nombre2, nom2);
    }
    ...
    strcpy(registros[*num_registros].ApPat, AP);
    validarAP(AP);
    quitarespacio(AP);
    strcpy(tem1, AP);
    ...
    strcpy(registros[*num_registros].ApMat, AM);
    quitarespacio(AM);
    if (strlen(AM) == 0)
    {
        strcpy(AM, "X");
